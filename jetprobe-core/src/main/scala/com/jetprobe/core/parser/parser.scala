...
class SQLParser extends StandardTokenParsers {
  ...
  class SqlLexical extends StdLexical {
    ...
    override def token: Parser[Token] =
      ( identChar ~ rep( identChar | digit | elem('.') )              ^^ { case first ~ rest => processIdent(first :: rest mkString "") }
  ...
  def ignoreCase(str : String) : String = str.toLowerCase
  ...
  def extract(stmt : SelectStmt) : ParsedSelect = {
    ...
    val table = stmt.relations.map(relns => relns.head match {
      case TableRelationAST(name,_,_) => name
    }).get

    ParsedSelect(fields,table)
